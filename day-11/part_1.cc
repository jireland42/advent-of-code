...
int main(int argc, char* argv[])
{
	...
	for (auto is_column_empty : empty_columns)
	{
		...
		if (is_column_empty) { cumulative_shift += 1; }
	}
	...
	for (size_t i{0}; i < galaxies.size(); i += 1) { galaxies[i].col += cumulative_shifts[galaxies[i].col]; }
